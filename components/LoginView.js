  ...
  return html`
    <div class="login-container h-screen w-full bg-[#0a0d18] flex flex-col md:flex-row overflow-hidden select-none text-slate-200">
      ...
      <div class="w-full md:w-1/2 h-full flex flex-col justify-center items-center p-6 sm:p-12 md:p-16 bg-[#131929] relative overflow-y-auto overflow-x-hidden">
        <div class="absolute w-[25rem] h-[25rem] bg-[#8efad4]/10 rounded-full blur-[100px] -top-10 -left-10 pointer-events-none"></div>
        <div class="absolute w-[25rem] h-[25rem] bg-[#3b82f6]/0.08 rounded-full blur-[100px] top-1/3 -right-20 pointer-events-none"></div>
        <div class="absolute w-[25rem] h-[25rem] bg-[#f97316]/0.08 rounded-full blur-[100px] -bottom-10 left-10 pointer-events-none"></div>
        <div class="absolute w-[25rem] h-[25rem] bg-[#a855f7]/15 rounded-full blur-[100px] bottom-10 right-10 pointer-events-none"></div>
        ...
        <div class="relative z-10 w-full max-w-md flex flex-col space-y-6">
          ...
          <div class="flex flex-col items-center text-center space-y-3">
            ...
            <div class="mt-4 flex flex-col items-center">
              <img src="./images/trofeo_mundial.png" class="h-20 w-auto object-contain drop-shadow-[0_4px_10px_rgba(142,250,212,0.4)] mb-3 animate-pulse" alt="Trophy" />
              <h2 class="text-xl font-bold font-outfit text-white tracking-tight uppercase leading-none">Iniciar Sesión</h2>
              <p class="text-[10px] text-[#8efad4] font-bold mt-2 uppercase tracking-widest">Plataforma de Pronósticos Oficial</p>
            </div>
          </div>

          ${recoveryStatus && html`
            <div class="p-3 text-xs font-bold rounded-xl border text-center ${recoveryStatus.type === 'error' ? 'bg-rose-500/10 border-rose-500/30 text-rose-400' : 'bg-[#8efad4]/10 border-[#8efad4]/30 text-[#8efad4]' }">
          ...
          <form onSubmit=${handleSubmit} class="space-y-4">
            ...
            <div class="space-y-1.5 text-left">
              ...
              <div class="flex items-center justify-between mt-1 select-none">
                <div></div>
                <button type="button" onClick=${handleRecuperar} disabled=${loading} class="text-[10px] text-slate-400 hover:text-[#8efad4] transition-colors font-bold uppercase tracking-wider bg-transparent border-0 cursor-pointer disabled:opacity-50 focus:outline-none">
                  ¿Olvidaste tu contraseña?
                </button>
              </div>
            </div>

            <button type="submit" disabled=${loading} class="w-full bg-[#8efad4] hover:bg-[#73e5bf] text-[#0a0d18] font-black rounded-xl py-3.5 text-xs uppercase tracking-widest transition-all duration-300 disabled:opacity-50 mt-2 flex items-center justify-center border border-[#8efad4] shadow-md hover:shadow-lg focus:outline-none">
              ${loading ? html`<div class="w-5 h-5 border-2 border-[#0a0d18] border-t-transparent rounded-full animate-spin"></div>` : html`<span>INGRESAR</span>`}
            </button>
          </form>
